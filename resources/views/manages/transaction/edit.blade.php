...
@section('process', __('app.buttons.edit').' '.__('app.menus.transaction'))
@section('main')
    <div class="wrapper">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="card card-primary">
                  ...
                  <div class="card-body">
                    <form action="{{ url(strtolower(__('app.menus.transaction')).'s/'.$field->id) }}" method="POST" enctype="multipart/form-data">
                      @method('patch')
                      @csrf
                      <x-select name="paket_code" id="paket_code" class="form-control" classForm="form-group">
                        ...
                        @foreach ($transactionDatas[0] as $paketCode)
                            @foreach ($paketCode as $item)
                                <option value="{{ $item->code }}" {{ $item->code == $field->paket_code ? 'selected' : '' }}>
                                  {{ $item->type }}
                                </option>
                            @endforeach
                        @endforeach
                      ...
                      <x-select name="user_code" id="user_code" class="form-control" classForm="form-group">
                        ...
                        @foreach ($transactionDatas[1] as $userCode)
                            @foreach ($userCode as $item)
                                <option value="{{ $item->code }}" {{ $item->code == $field->user_code ? 'selected' : '' }}>
                                  {{ $item->email }}
                                </option>
                            @endforeach
                        @endforeach
                      ...
                      <x-select name="template_code" id="template_code" class="form-control" classForm="form-group">
                        ...
                        @foreach ($transactionDatas[2] as $templateCode)
                            @foreach ($templateCode as $item)
                                <option value="{{ $item->code }}" {{ $item->code == $field->template_code ? 'selected' : '' }}>
                                  {{ $item->name }}
                                </option>
                            @endforeach
                        @endforeach
                      ...
                      <x-input type="date" name="order" placeholder="order" class="form-control" classForm="form-group" value="{{ $field->order ?? old('order') }}"/>

                      <x-input type="date" name="usage" placeholder="usage" class="form-control" classForm="form-group" value="{{ $field->usage ?? old('usage') }}"/>
