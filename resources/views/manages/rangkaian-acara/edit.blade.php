...
@section('process', __('app.buttons.edit').' '.__('app.menus.rangkaian_acara'))
@section('main')
    <div class="wrapper">
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="card card-primary">
                  ...
                  <div class="card-body">
                    <form action="{{ url(strtolower(__('app.menus.rangkaian_acara')).'s/'.$field->id) }}" method="POST">
                      @method('patch')
                      @csrf
                      <x-select name="akad_nikah_code" id="akad_nikah_code" class="form-control" classForm="form-group">
                        ...
                        @foreach ($rangkaianAcaraDatas[0] as $akadNikahCode)
                            @foreach ($akadNikahCode as $item)
                                <option value="{{ $item->code }}" {{ $field->akad_nikah_code == $item->code ? 'selected' : '' }}>
                                  {{ $item->place }}
                                </option>
                            @endforeach
                        @endforeach
                      ...
                      <x-select name="resepsi_code" id="resepsi_code" class="form-control" classForm="form-group">
                        ...
                        @foreach ($rangkaianAcaraDatas[1] as $resepsiCode)
                            @foreach ($resepsiCode as $item)
                                <option value="{{ $item->code }}" {{ $field->resepsi_code == $item->code ? 'selected' : '' }}>
                                  {{ $item->place }}
                                </option>
                            @endforeach
                        @endforeach
