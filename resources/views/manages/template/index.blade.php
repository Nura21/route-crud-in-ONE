      ...
      @section('process', __('app.buttons.list'). ' '.__('app.menus.template'))
      @section('main')
          <div class="wrapper">
                          ...
                          <table class="table table-head-fixed text-nowrap table-responsive" id="datatable">
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td>{{ $template->code ?? ''}}</td>
                                <td>{{ $template->name ?? ''}}</td>
                                <td class="col-md-12">{{ $template->type ?? ''}}</td>
                                <td>
                                  <a href="{{ url(strtolower(__('app.menus.template')).'s/'.$template->id.'/'.strtolower(__('app.buttons.edit'))) }}" class="btn btn-warning btn-xl" role="button">{{ __('app.buttons.edit') }}</a>
                                  <form action="{{ url(strtolower(__('app.menus.template')).'s/'.$template->id) }}" method="post">
                                    @method('delete')
                                    @csrf
